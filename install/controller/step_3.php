...
class ControllerStep3 extends Controller {
	...
	private function validate() {
		if (!$this->request->post['db_host']) {
			$this->error['db_host'] = 'Хост обязателен!';
		}

		if (!$this->request->post['db_user']) {
			$this->error['db_user'] = 'Имя пользователя обязательно!';
		}

		if (!$this->request->post['db_name']) {
			$this->error['db_name'] = 'Название базы данных обязательно!';
		}
		...
		if (!$this->request->post['password']) {
			$this->error['password'] = 'Пароль обязателен!';
		}

		if ((utf8_strlen($this->request->post['email']) > 96) || !preg_match('/^[^\@]+@.*\.[a-z]{2,6}$/i', $this->request->post['email'])) {
			$this->error['email'] = 'Не верный E-Mail!';
		}

		if (!$connection = @mysql_connect($this->request->post['db_host'], $this->request->post['db_user'], $this->request->post['db_password'])) {
			$this->error['warning'] = 'Ошибка: Невозможно соединиться к базе данных, пожалуйста убедитесь, что название базы данных, пароль, пользователь введены правильно!';
		} else {
			if (!@mysql_select_db($this->request->post['db_name'], $connection)) {
				$this->error['warning'] = 'Ошибка: База данных не создана!';
			}
		...
		if (!is_writable(DIR_OPENCART . 'config.php')) {
			$this->error['warning'] = 'Ошибка: Невозможна запись в файл сonfig.php проверьте права на следующий файл: ' . DIR_OPENCART . 'config.php!';
		}
	
		if (!is_writable(DIR_OPENCART . 'admin/config.php')) {
			$this->error['warning'] = 'Ошибка: Невозможна запись в файл сonfig.php проверьте права на следующий файл: ' . DIR_OPENCART . 'admin/config.php!';
		}	
