...
class ControllerCheckoutCart extends Controller {
	...
	public function index() {
    	...
    	if ($this->cart->hasProducts() || !empty($this->session->data['vouchers'])) {
			...
			$this->data['confirm_action'] = $this->url->link('checkout/confirm');
			...
			$products = $this->cart->getProducts();
      		foreach ($products as $product) {
				...
				if ($product['image']) {
                    $imagesize = getimagesize(DIR_IMAGE . $product['image']);
				    $image = $this->model_tool_image->resize($product['image'], ($imagesize[0] / 2), ($imagesize[1] / 2));
				} else {
					$image = '';
				}
            ...
            $this->data['order_blocked'] = false;
			if($total < 1){
			    $this->data['order_blocked'] = true;
            }
			$this->data['min_total'] = 1;
			...
			if(isset($this->request->get['error_fio'])){
			    $this->data['errors']['fio'] = $this->request->get['error_fio'];
            }
			if(isset($this->request->get['error_email'])){
			    $this->data['errors']['email'] = $this->request->get['error_email'];
            }
			if(isset($this->request->get['error_phone'])){
			    $this->data['errors']['phone'] = $this->request->get['error_phone'];
            }
