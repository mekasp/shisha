	...
	public function getStockStatuses($data = array()) {
	    if ($data) {
			...
			if (!$stock_status_data) {
			    $query = $this->db->query("SELECT ss.stock_status_id, ssd.`name` FROM " . DB_PREFIX . "stock_status ss LEFT JOIN " . DB_PREFIX . "stock_status_description ssd ON (ss.stock_status_id = ssd.stock_status_id) WHERE ssd.language_id = '" . (int)$this->config->get('config_language_id') . "' ORDER BY ss.`stock_status_id` DESC");
